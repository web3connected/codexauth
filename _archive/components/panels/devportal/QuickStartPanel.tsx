...
const codeExamples: Record<Language, string> = {
  javascript: `import { CodexHash } from '@web3connected/codexhash-starter-kit';
...
import "github.com/web3connected/codexhash-go"

func main() {
    hasher := codexhash.New(codexhash.Options{
        Algorithm:     "harmonic",
        SecurityLevel: "quantum-resistant",
    })
  ...
  rust: `use codexhash::CodexHash;

fn main() {
    let hasher = CodexHash::new()
        .algorithm("harmonic")
        .security_level("quantum-resistant")
        .build();
      ...
      <div className="container mx-auto px-4">
        <div className="max-w-5xl mx-auto">
          ...
          <div className="mt-8 grid md:grid-cols-2 lg:grid-cols-4 gap-4">
            <InstallCommand
              language="npm"
              command="npm install @web3connected/codexhash-starter-kit"
            />
            <InstallCommand
              language="pip"
              command="pip install codexhash"
            />
            <InstallCommand
              language="go"
              command="go get github.com/web3connected/codexhash-go"
            />
            <InstallCommand
              language="cargo"
              command="cargo add codexhash"
            />
          </div>
        </div>
      </div>
...
function InstallCommand({ language, command }: { language: string; command: string }) {
  ...
  return (
    <div className="flex items-center gap-2 px-4 py-3 rounded-lg bg-white/5 border border-white/10 group">
      <span className="text-xs text-hash-primary font-semibold uppercase">{language}</span>
