...
export interface SplitTextStatsPanelProps {
  ...
  title?: string;
  paragraphs: string[];
  /** Optional word/phrase in paragraphs to highlight with auth-primary color */
  highlight?: string;
  statCards: StatCard[];
}
        ...
        <div className="max-w-6xl mx-auto grid md:grid-cols-2 gap-16 items-start">
              ...
              if (highlight && para.includes(highlight)) {
                ...
                return (
                  <p key={i} className="text-slate-400 leading-relaxed mb-4">
                    {before}
                    <span className="text-auth-primary font-medium">{highlight}</span>
                    {after}
                  </p>
                );
              }
          ...
          <div className="space-y-4">
            {statCards.map((card) => (
              <div
                key={card.value}
                className="group relative p-6 rounded-2xl border border-white/10 bg-white/5 backdrop-blur-sm hover:bg-white/8 hover:border-auth-primary/30 transition-all"
              >
                <div className="absolute top-0 left-6 right-6 h-px bg-gradient-to-r from-transparent via-auth-primary/40 to-transparent" />
                <div className={`text-3xl font-bold font-mono mb-1 ${card.color ?? 'text-auth-primary'}`}>
                  {card.value}
                </div>
