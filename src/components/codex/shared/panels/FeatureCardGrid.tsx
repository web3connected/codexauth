...
export const FeatureCardGrid: React.FC<FeatureCardGridProps> = ({
  ...
  return (
    <section className="py-20 lg:py-28 bg-gradient-to-b from-transparent via-auth-primary/5 to-transparent">
      <div className="container mx-auto px-4">
        ...
        {viewAllHref && (
          <div className="text-center mt-12">
            <a
              href={viewAllHref}
              className="inline-flex items-center gap-2 text-auth-primary hover:text-auth-primary/80 font-medium transition-colors"
            >
              {viewAllLabel}
...
function SDKCardItem({ sdk }: { sdk: SDKCard }) {
  return (
    <a
      href={sdk.href}
      className="group block p-6 rounded-2xl border border-white/10 bg-white/5 backdrop-blur-sm transition-all hover:bg-white/10 hover:border-auth-primary/30 hover:scale-[1.02]"
    >
      <div className="flex items-start gap-4 mb-4">
        <div className="p-3 rounded-xl bg-white/5 border border-white/10 group-hover:border-auth-primary/20 transition-colors shrink-0">
          {sdk.icon}
        </div>
        <div className="flex-1 min-w-0">
          <h3 className="text-lg font-semibold text-white group-hover:text-auth-primary transition-colors">
            {sdk.name}
          </h3>
      ...
      <div className="flex items-center justify-between">
        ...
        <span className="text-sm text-slate-500 group-hover:text-auth-primary transition-colors flex items-center gap-1">
          Learn more
          <svg
